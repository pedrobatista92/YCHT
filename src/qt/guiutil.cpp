...
{
    // Convert YCHT:// to YCHT:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if (uri.startsWith("YCHT://"))
    {
        uri.replace(0, 12, "YCHT:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "YCHT.lnk";
}
...
{
    return GetAutostartDir() / "YCHT.desktop";
}
...
{
    header = tr("YCHT-qt") + " " + tr("version") + " " +
             QString::fromStdString(FormatFullVersion()) + "\n\n" +
             tr("Usage:") + "\n" +
             "  YCHT-qt [" + tr("command-line options") + "]                     " + "\n";
